...
int desc(node *n) {
    if (!n) return 0;
    int descendants = 0;
    for (size_t i = 0; i < n->children.getSize(); i++) {
        descendants += 1 + desc(n->children[i]);
    }
    return descendants;
}


void inject(node *r, node *v) {
    if (r->index == -1 || depth(r) < d) {
        CustomList<node *> C = r->children;
        int g = INT_MAX;
        node *t = nullptr;


        size_t childCount = C.getSize();
        for (size_t i = 0; i < childCount; i++) {
            node *c = C[i];
            if (dominates(c->index, v->index, CustomList<int>())) {
                int descCount = desc(c);
                if (descCount < g) {
                    t = c;
                    g = descCount;
                }
    ...
    if (r == nullptr || depth(r) < d) {
        CustomList<node *> C = r->children;

        for (int i = r->children.getSize() - 1; i >= 0; i--) {
            node *c = r->children[i];
            if (c != v && dominates(v->index, c->index, CustomList<int>())) {
                r->children.removeByIndex(i);
                c->parent = v;
                v->children.insert(c);
            }
        }
...
int main() {
    ...
    CustomList<CustomList<CustomList<int>>> times = readTimesFromFile("dataset/" + inputSize + "/" + inputSize + ".times");
